...
async def admin_risghts(client: Client, CallbackQuery):
  try:
    ...
    if command == "stop":
        ...
        await CallbackQuery.answer("تم انهاء التشغيل لعدم وجود مستمعين ♥️🌿", show_alert=True)
  ...
  try:
    ...
    if command == "/pause" or command == "ايقاف مؤقت":
        ...
        try:
         ...
         pass
        await remove_active(bot_username, chat_id)
        await message.reply_text(f"**حاضر سكت اهو 😢♥️**")
