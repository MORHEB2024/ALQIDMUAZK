...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "Q_A_ID":
      try:
         ...
         await client.set_administrator_title(chat_id, user_id, "قائد")
      except:
        pass
      return await message.reply_text(f"**♪ انضم المطو قائد للشات  🚦 .\n♪ مرحبا بك : @Q_A_ID  🚦 .**")
    dev = await get_dev(bot_username)
...
async def Q_A_ID(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
"مره واحد اسمو قائد  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
...
"لا القائد على الدوق فيما يحب أو بكره",
...
async def alive(client: Client, message):
    ...
    alive = f"""╭──── • ◈ • ────╮
么 [𝗦𝗢𝗨𝗥𝗖𝗘 𝗦𝗨𝗡𝗗𝗘𝗥𝗟𝗔](t.me/SO_Qaid) 🚦 .
么  [𝒅𝒆𝒗 𝐀𝐋𝐐𝐀𝐈𝐃 ](t.me/Q_A_ID) 🚦 .
╰──── • ◈ • ────╯
    ...
    await message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup(
                    [
                        ...
                        [
                            InlineKeyboardButton("SeMo ElKbEr 🥷", user_id=f"Q_A_ID")
                        ],
                        [
...
@Client.on_message(filters.command(["القائد","قائد","محمود","المبرمج"], ""))
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="Q_A_ID")
...
@Client.on_message(filters.command(["الالقائد","المطور حوده ","المطور القائد"], ""))
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["Q_A_ID"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["Q_A_ID"]:
   ...
   if message.from_user.username == "Q_A_ID":
     await message.reply_text(f"**♪ تم تنفيذ الامر بواسطة المطور قائد  🚦 .**")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @SO_Qaid  🚦 .**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @SO_Qaid  🚦 .**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @SO_Qaid  🚦 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["Q_A_ID"]:
         await message.reply_text("**♪ رتبتك هي : المطور قائد+  🚦 .**")
         return
     if userr.username in ["Q_A_ID"]:
         await message.reply_text("**♪ رتبتك هي : المطور القائد 🚦 .**")
         return
     if userr.id == dev:
